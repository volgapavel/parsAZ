"""Database repository for 1news.az news operations."""
import logging
...
CREATE_TABLE_SQL = """
CREATE TABLE IF NOT EXISTS onenews (
...
CREATE INDEX IF NOT EXISTS idx_onenews_pub_date ON onenews(pub_date);
"""

INSERT_NEWS_SQL = """
INSERT INTO onenews (link, pub_date, title, content)
VALUES %s
ON CONFLICT (link) DO NOTHING;
"""

CHECK_EXISTS_SQL = "SELECT 1 FROM onenews WHERE link = %s"
...
def init_schema(conn: Connection) -> None:
    ...
    logger.info("Database schema for onenews initialized")
...
def insert_single_news(conn: Connection, article: NewsArticle) -> bool:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "INSERT INTO onenews (link, pub_date, title, content) VALUES (%s, %s, %s, %s) ON CONFLICT (link) DO NOTHING",
                article.to_tuple()
            )
...
def get_news_count(conn: Connection) -> int:
    ...
    with conn.cursor() as cur:
        cur.execute("SELECT COUNT(*) FROM onenews")
        result = cur.fetchone()
